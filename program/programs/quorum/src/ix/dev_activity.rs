...
pub fn handler(ctx: Context<crate::RegisterDevActivity>) -> Result<()> {
    ...
    // Aplica durante la fase económica (que incluye el vesting bilateral)
    require!(
...
pub fn trigger_inactivity(ctx: Context<crate::TriggerInactivity>) -> Result<()> {
    let project = &mut ctx.accounts.project;
    let now = Clock::get()?.unix_timestamp;
