...
pub fn handler(ctx: Context<crate::CastSocialVote>) -> Result<()> {
...
pub fn open_economic_phase(ctx: Context<crate::OpenEconomicPhase>) -> Result<()> {
    let project = &mut ctx.accounts.project;
    let now = Clock::get()?.unix_timestamp;
...
pub fn close_social_phase(ctx: Context<crate::CloseSocialPhase>) -> Result<()> {
    let project = &mut ctx.accounts.project;
    let now = Clock::get()?.unix_timestamp;
